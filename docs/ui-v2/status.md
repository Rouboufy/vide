...
| 04A Task-runner design record | Draft; review and approval pending | Prompts 00, 03, and 03B accepted | Draft introduced by `7252773`; revised in mixed reactor-review commits `098ed75`, `a9c83a2`, and `5b9cd91`, with child-ownership clarifications `b2c2bfe` and `6ebe6d3`; no implementation or PR | `docs/ui-v2/task-runner-04a.md`; pinned Zig 0.16.0 and local standard-library concurrency contract inspected; `git diff --check` | Started 2026-08-26; worker sizing, capacities, admission policy, notifier, ownership, cancellation, shutdown target, and rollback remain subject to approval | Documentation only; 04B gate remains closed; because the mixed revision commits also contain 03B fixes, rollback removes the 04A document and status entry as a content-level change instead of reverting those commits wholesale; no runtime or persisted-state impact |
