...
pub const ActivityBar = struct {
    ...
    pub fn handleMouse(self: *ActivityBar, mx: u16, my: u16, rect: Rect) ?usize {
        if (mx < rect.x or mx >= rect.x + rect.w or my < rect.y or my >= rect.y + rect.h) return null;

        for (items, 0..) |_, idx| {
            const start_y = rect.y + @as(u16, @intCast(idx)) * 3;
            if (start_y < rect.y + rect.h and my >= start_y and my < @min(start_y + 3, rect.y + rect.h)) {
                self.active_idx = idx;
                return idx;
            }
        }
