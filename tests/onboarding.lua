...
assert(vim.uv.fs_stat(vim.fn.stdpath('data') .. '/onboarding-complete'))
-- The welcome action must request Vide's native language-tools panel even
...
vim.rpcnotify = function(channel, method, ...)
  if method == 'vide_open_language_tools' then
    assert(channel == 1)
    requested = true
...
for _, mode in ipairs({ 'normal', 'ide' }) do
  _G.vide_onboarding_choose(mode)
  for _, key in ipairs({ '<CR>', '<Esc>', 'q', '<C-c>' }) do
    ...
    assert(vim.g.vide_ide_mode == (mode == 'ide'), 'closing changed editing style')
  end
end
vim.cmd('qa!')
