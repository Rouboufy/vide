...
| **Zig** | `>= 0.16.0 (master)` | Required to build from source — installed automatically by `setup.sh` or `setup_macos.sh` if missing |
| **git** | any | Required to clone the repository |
| **curl** | any | Required by installer scripts |
| **unzip** | any | Required to install the Nerd Font |
| **A Nerd Font** | — | JetBrainsMono Nerd Font installed automatically if missing |
...
Run the one-line installer for your platform. It will automatically:
