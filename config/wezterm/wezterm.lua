...
wezterm.on("update-status", function(window, pane)
  ...
  -- Read config state from preview.json first, then settings.json
  local state_path = wezterm.home_dir .. "/.local/share/vide/preview.json"
  local file = io.open(state_path, "r")
  if not file then
    state_path = wezterm.home_dir .. "/.local/share/vide/settings.json"
    file = io.open(state_path, "r")
  end
  if file then
    ...
    if content then
      local theme = content:match('"theme":"([^"]+)"')
